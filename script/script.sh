...
count=0
while [ "$( curl -s http://${HUB_IP:-grid_hub}:4444/status | jq -r .value.ready )" != "true" ]
do
  count=$((count+1))
  echo "Attempt: ${count}"
  if [ "$count" -ge 30 ]
...
java -cp 'libs/*' \
     ...
     -Dselenium.grid.urlFormat="http://${HUB_IP:-grid_hub}:4444/wd/hub" \
